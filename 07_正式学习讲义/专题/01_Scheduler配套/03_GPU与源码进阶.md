...
**健康变化练习：**8 张独占 GPU 中一张被插件报告 unhealthy，kubelet 会减少 GPU allocatable，capacity 不因此减少。已使用故障卡的 Pod 不会自动换卡或重新调度。应对齐 UUID、插件健康报告、Pod 使用设备和应用错误；不能因 capacity 仍为 8 就判断故障报告未生效。[G1]
...
排查按顺序走：Pod 引用谁 → Claim 是否同命名空间且存在 → Class 是否匹配供给 → allocation 是否生成 → Pod 是否绑定 → 节点驱动是否准备成功。API discovery 只证明 API 被服务，不能证明设备供给链正常。
...
`podRequest` 与 `nodeInfo` 来自函数参数，前者已汇总 Pod 请求，后者提供候选节点账；`insufficientResources` 是前文初始化的失败记录切片。摘录前是 Pod 数检查，后面继续检查内存等资源，均不在本块内。
...
**大白话总结：**输入是新需求和节点请求账；比较的是需求与余额；不足则追加失败记录，没有在此处删除任何 Pod。代入 `8000m 可分配、7000m 已请求、2000m 新请求`，余额只有 1000m，因此失败，但 Unresolvable=false。新请求改成 9000m，则为 true：仅删除别的 Pod 不能让该节点变成 9 核节点，不代表扩容或改变需求也无解。

**顺手学 Go：**`&&` 是且，左边不成立就不算右边；`对象.方法()` 读取该对象提供的值；`类型{字段: 值}` 构造结构体；切片是可变长序列，`append` 返回更新后的切片，故需要重新赋值。这里没有 return，所以 CPU 失败后还能检查内存。

回到完整 Filter：它先调用 fitsRequest；读取前置状态出错会返回 Error；资源不足记录会转成 Status 和原因，任何记录的 Unresolvable 为 true 时，固定快照选用 UnschedulableAndUnresolvable；没有不足时返回 nil，按此接口表示成功。Framework 收到失败后才停止此节点后续 Filter 插件。这解释了“一个插件能累计多个资源原因”与“插件之间会短路”并不矛盾。

Unresolvable 字段和该状态分流属于本课固定快照，不能直接断言实验 v1.34 或所有生产版本都具有同一实现。真实源码与许可证归 Kubernetes 上游，见[第三方说明](../../../THIRD_PARTY_NOTICES.md)。
...
- [G12 GPU 资源请求规则](https://kubernetes.io/docs/tasks/manage-gpus/scheduling-gpus/)
- [G13 NVIDIA Device Plugin 配置](https://github.com/NVIDIA/k8s-device-plugin#configuration-option-details)
- [G14 固定整数评分](https://github.com/kubernetes/kubernetes/blob/301946d15e67a4a2e8a5fb8292eb836acd366d78/pkg/scheduler/framework/plugins/noderesources/most_allocated.go)；[评分输入](https://github.com/kubernetes/kubernetes/blob/301946d15e67a4a2e8a5fb8292eb836acd366d78/pkg/scheduler/framework/plugins/noderesources/resource_allocation.go)
- [G15 v1.34 DRA 对象与生命周期](https://v1-34.docs.kubernetes.io/docs/concepts/scheduling-eviction/dynamic-resource-allocation/)
