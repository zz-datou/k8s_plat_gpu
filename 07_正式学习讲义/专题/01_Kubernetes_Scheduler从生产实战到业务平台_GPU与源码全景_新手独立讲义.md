...
> 2026-09-08 第二轮审校版。本文是主教材，不再要求新手先读完整源码调用链。
>
...
Deployment 通常通过 ReplicaSet 管理 Pod：ReplicaSet 维持副本数量，scheduler 为已创建的 Pod 选节点，kubelet 再启动容器。创建副本和选择节点是两件事。

阅读 YAML 先分三栏：`metadata` 放名称、标签、UID 等身份信息；`spec` 描述配置；`status` 记录组件报告的当前状态。例如 `spec.nodeName` 是已分配节点，`status.conditions` 帮助判断推进到哪一步。模板说明准备创建什么，最终 Pod 才是准入处理后实际交给 scheduler 的输入。

**标签**是对象上的键值标记，**selector** 是筛选规则。Node 标签辨认节点池或可用区；Pod 标签辨认应用副本，两者不能互相代替。Namespace 是对象的命名与管理范围，不自动提供独占节点。后面遇到 PVC、PriorityClass 和插件时再分别解释，不要求先背完所有对象。
...
UID 用来区分对象实例，避免把重建前后的事件拼成同一事故；Event 会聚合、过期或缺失，因此“没有搜到”不等于“从未发生”。Condition 给当前汇总状态，也不等于完整历史。[S3]
...
**第二遍再读：原生 sidecar 如何按阶段相加。**假定 init 列表依次是一个 `restartPolicy: Always` 的日志 sidecar（200m/256Mi）、一个普通初始化容器（1500m/512Mi）；业务容器为 1000m/1Gi，无其他资源设置。启动 sidecar 时是 200m/256Mi；它与普通 init 同时运行时是 1700m/768Mi；业务运行时是 1200m/1280Mi。逐资源取峰值，得到 1700m/1280Mi，再加适用的 overhead。原生 sidecar 的顺序会影响哪些初始化阶段重叠；不要把全部 init 一律相加，也不要全部只取单个最大值。[S16]
...
eligible domains 是按该规则参与统计的拓扑域。global minimum 来自这些域；域数小于指定 minDomains 时，global minimum **按 0 处理**。它不是任何场景都可替换成“最终全局最大数减最小数”。selector、namespace、nodeAffinityPolicy、nodeTaintsPolicy 和域标签会改变统计范围。[S7]
...
PriorityClass 是集群级优先级配置，Pod 通过 priorityClassName 引用。通常更高优先级更早获得尝试机会，但退避、gate 和当前队列规则仍会影响时机；正常约束判断没有可行节点时，抢占机制可能模拟移除较低优先级 Pod，寻找未来可行的方案。实际删除、优雅终止、资源释放和后续重新调度不是同一个动作。[S8]
...
CPU 不够时每毫秒重试并不会多出 CPU，只会浪费控制面。这里的“事件”首先指 API 对象变化经 watch/informer 传播的通知，和 `kubectl get events` 返回的 Kubernetes Event 对象不是一回事。相关 Node/Pod 变化可能让失败条件改变，QueueingHint 用于判断是否值得重算，而不是保证下次一定成功。调度过程中的事件跟踪还要避免“释放发生在计算期间，失败落队后却错过唤醒”的竞态。[S17]
...
自评 10 分：责任方 2 分、单位与请求计算 2 分、硬条件交集 2 分、证据时间线 2 分、修复与验收 2 分。达到 8 分且没有关键误判，再进生产设计篇。把“Pod 不存在”判为资源 Filter 失败、用 usage 代替请求、把 Ready 当业务健康、把硬分散当跨区容量保证，都应返回相应章节补练。
